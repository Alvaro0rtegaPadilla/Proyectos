...
.then(data => {
    ...
    data.items.forEach(category => { 
        ...
        let tableBody = '';
        // Como funciona? para cada categoria en los datos, se realizan varios pasos: se crea un elemento <h2> para el titulo de la categoria, se establece su contenido de texto al nombre de la categoria (category.category), y luego se agrega al contenedor del menu.
        ...
        //Que hace? una vez complementadas todas las filas de la tabla para los elementos de una categoria, se combina el encabezado de la tabla (tableHead) con el cuerpo de la tabla (tableBody) y se establece como el contenido HTML de la tabla. Finalmente, esta tabla se anade al contenedor del menu en la pagina web. 
        menuContainer.appendChild(table);
    });
});
//Poblar la tabla con los elementos: Para cada item dentro de una categoria, se crea una fila (<tr></tr>) con tres celdas(<td></td>): una para la imagen (con un elemento <img>), otra para el nombre y descripcion del item, y una ultimapara el precio. Esto se hace concatenando la nueva fila a una variable tableBody.
//Finalizar y mostrar la tabla: Una  vez que todas las filas han sido agregadas a tableBody, se establece el contenido interno (innerHTML) de la tabla combinando el encabezado con el cuerpo. Luego , esta tabla  completa se agrega el contenedor del menu en el documento. 
